package com.product.producer.web.rest;
...
import com.product.producer.IntegrationTest;
import com.product.producer.config.EmbeddedKafka;
...
class ProductProducerKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/product-producer-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/product-producer-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/product-producer-kafka/unregister"));
                return;
            }
        }
